...
v4.4 将 v4.3 定义的 FPN/TPN 接口集成到 agent.step() 主循环: FPN 探照灯增益调制感觉输入、TPN↔DMN 跷跷板动态由 ACC 冲突信号驱动、修复 cns 循环导入。
...
数据流 (v4.4): `s → Hippocampus.learn(s) → Cingulate.compute_F(z,s) → TPN.receive_salience(conflict, novelty, urgency) → TPN.update_seesaw() → FPN.gate_attention(s) → Prefrontal.select_action() → a → MetaLearner.update(F)`

自下而上（感官→皮层）：`s → V1 → V2 → V4 → IT → 海马/杏仁核 → ACC` — 快速粗糙前馈
自上而下（预期→感官）：`前额叶 → FPN(探照灯) → V4/V2/V1 (增益调制)` — 慢速精确反馈
跷跷板切换 (v4.4 已集成): `Cingulate(冲突/新颖/紧迫) → TPN.receive_salience() → TPN.update_seesaw() → TPN↑/DMN↓ (任务) 或 DMN↑/TPN↓ (走神)`
