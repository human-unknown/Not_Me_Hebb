...
**当前版本: v5.5 — 神经调节系统 (下丘脑 + VTA RPE + 蓝斑核 NE)**

v5.3 实现真实音频输入支持 (替换 v5.2 语义代理模式)，
AudioInput 模块支持 WAV/MP3/FLAC 加载和麦克风录制，
32 通道 Mel 频谱驱动全听觉层级管线，
立体声支持双耳定位 (独立左右耳频谱 → SOC ITD/ILD)。
感知维度 D_V52=468 (听觉段 96d 不变)。
...
数据流 (v5.5): `s[text] → 听觉层级(Phase 0b) + 视觉层级(Phase 0) + 痛觉层级(Phase 0c) → Hypothalamus(Phase 0d: 稳态驱动) → VTA(Phase 0e: RPE→学习率) → LC(Phase 0f: NE→SNR+RVM) → Hippocampus.learn(s) → Cingulate.compute_F(z,s) → TPN.receive_salience() → TPN.update_seesaw() → FPN.gate_attention(s) → Prefrontal.select_action() → a → MetaLearner.update(F)`
...
- **感知维度 D = 468 (D_V52)**（text[64] | vision[308] | audio[96]）
- **隐状态 H = 16**
- **最大簇数 K = 256**
