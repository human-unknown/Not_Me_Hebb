...
v4.3 对齐图3六大运行规则：新增 FP N/TPN 网络、完善六条核心原则、引入 Hebb LTP/LTD 机制描述。
自 v4.2: V1/V2/V4 独立模块、消除所有 flat import、删除根目录 shim 层。所有导入 100% 使用层级包路径。
...
- 自上而下: 前额叶 `prefrontal.py` 的 EFE 行动选择 ← 扣带回 `cingulate.py` 的自由能评估（已实现）；FPN `fpn.py` 注意力模板 → 感觉皮层增益调制（v4.3 新增接口）
...
**项目实现**：
- 感知向量 s ∈ R^330 本身是分布式表征: text[64] | V1[96] | V2[64] | V4[64] | Color[42]
...
- FPN: `fpn.py` FrontoparietalNetwork — 注意力探照灯、工作记忆 (v4.3 ★ NEW)
- TPN: `tpn.py` TaskPositiveNetwork — TPN↔DMN 跷跷板动态、认知努力、任务疲劳 (v4.3 ★ NEW)
...
| **FPN (额顶网络)** | `cerebrum/association/fpn.py` | ★ NEW v4.3 选择性注意探照灯、工作记忆 (图3 规则4) |
| **TPN (任务正网络)** | `cerebrum/association/tpn.py` | ★ NEW v4.3 任务执行、DMN跷跷板对立 (图3 规则4) |
...
数据流：`s → [Thalamus.gate(s)] → Hippocampus.learn(s) → Cingulate.compute_F(z,s) → [FPN.attention_searchlight()] → Prefrontal.select_action() → a → MetaLearner.update(F)`

自下而上（感官→皮层）：`s → V1 → V2 → V4 → IT → 海马/杏仁核 → ACC`
自上而下（预期→感官）：`前额叶 → FPN(探照灯) → V4/V2/V1 (增益调制)`
跷跷板切换：`Cingulate(冲突) → TPN.receive_salience() → TPN↑/DMN↓ (任务) 或 DMN↑/TPN↓ (走神)`
...
### 方法3: 无 LLM (No Large Language Models)
...
| **v4.3** | 规则更新 — 对齐图3六大规则、FPN/TPN新增、LTP/LTD机制、注意力瓶颈 | 本版本 |
