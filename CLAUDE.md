...
- **L0 ClusterNetwork**：细胞集群 (cell assembly) 记忆，无暴力扫描，特征哈希定位 O(1) + 桶内竞争
- **集群形成** = 学习（新感知创建簇）
- **集群激活** = 回忆（输入部分匹配 → 簇被激活 → 侧抑制竞争）
- **集群衰减** = 遗忘（未使用的簇逐渐衰减）
- **睡眠巩固**：每 100 步清理弱簇
- **词序网络 (Broca 区)**：双 Hebb 架构——概念网络管"想什么"，词序网络管"怎么说"。v5: `speak_from_state()` 词序 Hebb 链逐词生成，不再引用整句语料
- 不预设符号，一切知识从统计共现中生长
...
| **L2** | `layer2_inference.py` | 前额叶 | 期望自由能 (EFE) 驱动行动选择、MoE 门控、社会信念更新 |
| **L2.5** | `layer2_5_moe.py` | 基底节 | 动作门控 (action gating)，决定何时表达 |
| **L3** | `layer3_meta.py` | 神经调节系统 | 元学习、关键期、可塑性衰减、创伤模拟 |
| **Broca** | `broca.py` | 布罗卡区 | 词序学习、语言产出、v5: `speak_from_state()` 逐词 Hebb 链生成 |
...
- 整句检索而非 token 生成（`broca.speak_sentence()`）
- 嵌入使用轻量级 sentence-transformer (`all-MiniLM-L6-v2`)，仅用于语义编码，不参与认知加工
- 说话 = 从集群记忆中检索与当前信念状态最相似的听过的话
...
├── utils.py               # 工具函数
│
├── broca.py               # Broca 区 — v5: speak_from_state() 词序 Hebb 链生成 + speak_sentence() 整句检索回退
...
| **Stage 6** | 人机对话闭环 — 人类输入 s[0:64]，Agent 整句回应 | `python main_dialogue.py` |

## 核心技术参数

- **感知维度 D = 330**（text[64] | vision[64] | audio[64] | body[64] | meta[64] | action-consequence[10]）
- **隐状态 H = 16**
- **最大簇数 K = 256**
- **行动数 A = 5** (N, S, W, E, REST)
...
```bash
python stage2_crossmodal.py --dataset coco --n 5000 --mode all  # Stage 2 跨模态
python main.py                    # M1 默认
python main.py --m2               # M2 视野遮蔽
python main.py --m3               # M3 多智能体
python main.py --m4               # M4 参数扫描
python main.py --m5               # M5 长程发育
python main_dialogue.py           # Stage 6 人机对话
```
