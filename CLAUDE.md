...
v4.1 基于图2 (大脑各结构功能分工) 进一步细化脑区架构，将丘脑从边缘系统独立为 Level 3 结构。
...
v4.1 根目录 `.py` 文件已转换为 **re-export shim**——薄封装层，将符号从 cerebrum/cns/brainstem_cerebellum 层级重导出。

- 根目录 shim (例: `from agent import Agent` → `from cns.agent import Agent`)
- `python entry/main_dialogue.py` 仍可直接运行
- 权威代码位于 cerebrum/ / cns/ / brainstem_cerebellum/ 层级
- 所有层级内部 import 已更新为包路径 (cns.xxx, cerebrum.xxx)，消除对根目录 shim 的依赖
...
| **v4.1** | 大脑功能细分 — 丘脑独立、Gestalt恢复、架构对齐图2 | 本版本 |
...
- 本项目为研究和探索性质
- v4.0/v4.1 架构详见 `v4_architecture.md`
