"""spectrum_generator.py — 64d向量→梅尔频谱 MLP"""
import numpy as np, torch, torch.nn as nn, os
from torch.utils.data import DataLoader, TensorDataset

class SpectrumGenerator(nn.Module):
    def __init__(self, vec_dim=64, mel_bins=80, n_frames=64, hidden=256):
        super().__init__()
...
def train(data_path=None, epochs=500, batch_size=16, lr=1e-3):
    if data_path is None: data_path = os.path.join(os.path.dirname(__file__), 'spectrum_dataset.npy')
    raw = np.load(data_path, allow_pickle=True)
    vecs = np.stack([r[0] for r in raw]); mels = np.stack([r[1] for r in raw])
    print(f"Dataset: {len(vecs)} pairs, vec={vecs.shape}, mel={mels.shape}")
    ...
    torch.save(model.state_dict(), os.path.join(out,'spectrum_generator.pt'))
    np.save(os.path.join(out,'spectrum_norm.npy'), {'vec_mean':v_mean,'vec_std':v_std,'mel_min':mel_min,'mel_max':mel_max})
